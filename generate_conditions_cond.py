...
cov = np.ones((num_of_options, num_of_options))

f1 = np.random.uniform(size=num_of_options) * (max_f - min_f) + min_f
f2 = np.random.multivariate_normal(f1,cov) 
f3 = np.random.multivariate_normal(f2,cov)
...
D.to_excel('delay_disc_conditions_cond.xlsx',index=None,float_format = '%.3f',)
